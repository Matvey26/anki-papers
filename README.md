...
Новые выделения анализируются `gpt-5.6-luna` как `lemma + lexical family +
part of speech + exact sense`. Формы и близкие дериваты с общим смысловым
ядром объединяются: `acquired/acquisition` образуют семейство `acquire`, а
`acknowledge/acknowledging` — `acknowledge`. Luna расширяет общее определение
...
карточки. Ответ выбирается вместе с контекстом, поэтому карточка показывает
нужную форму, а не только имя семейства. Карточки используют отдельный тип
