...
Новые выделения анализируются `gpt-5.6-luna` как `lemma + part of speech +
exact sense`. Если в профиле уже есть такая же смысловая карточка, выделение
и один явно помеченный `llm_generated` B2+ контекст добавляются к ней. Если
смысл другой, создаётся отдельная карточка — одинаковое написание не является
основанием для объединения. Карточки используют отдельный тип заметок `Anki
Papers Semantic`; старые `Anki Papers` и их расписание не изменяются.
